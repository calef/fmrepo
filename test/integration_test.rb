...
class IntegrationTest < Minitest::Test
  ...
  def setup
    ...
    Place.bind(@repo)
    PlaceWithCustomRelation.bind(@repo)
  ...
  def test_order_sorts_by_field
    ...
    assert_equal ["Bellevue", "Seattle", "Spokane"], titles
  end

  def test_order_sorts_descending
    places = Place.order("title", :desc).to_a
    titles = places.map { |p| p["title"] }
    assert_equal ["Spokane", "Seattle", "Bellevue"], titles
  end
  ...
  def test_save_updates_existing_record
    place = Place.find("_places/seattle.md")
    place["population"] = 750000
    place.save!

    # Reload and verify
    reloaded = Place.find("_places/seattle.md")
    assert_equal 750000, reloaded["population"]
  end
